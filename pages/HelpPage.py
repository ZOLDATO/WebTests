...
class HelpPageHelper(BasePageHelper):
    ...
    def check_page(self):
        with allure.step('Проверяем корректность загрузки страницы Помощи'):
            self.attach_screenshot()  # скрин 1
            scroll_item = self.find_element(HelpPageLocators.GROUPS)
            ActionChains(self.driver).scroll_to_element(scroll_item).perform()
            self.attach_screenshot()  # скрин 2 после прокрутки страницы до середины
            scroll_item = self.find_element(HelpPageLocators.ADVERTISEMENT_CABINET)
            ActionChains(self.driver).scroll_to_element(scroll_item).perform()
            self.attach_screenshot()  # скрин 3 после прокрутки страницы до конца
